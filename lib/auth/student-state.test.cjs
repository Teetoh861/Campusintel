...
async function fixture(run) {
  ...
  const mocks = {
    ...
    '@/lib/supabase/auth-gateway': { getAuthGateway: () => ({
      ...
      resend: async () => { calls.push(['resend']); return { error: resendError } },
    }) },
...
test('unconfirmed login attempts one limited fresh resend and never claims delivery on failure', async () => fixture(async f => {
  const scenarios = [
    { delivery: 'fresh' },
    { delivery: 'limited', limitError: Object.assign(new Error('private'), { status: 429 }) },
    { delivery: 'limited', resendError: { code: 'over_email_send_rate_limit' } },
    { delivery: 'failed', resendError: { code: 'unexpected', message: 'private provider detail' } },
    { delivery: 'failed', limitError: new Error('private database failure') },
  ]
  for (const scenario of scenarios) {
    ...
    assert.deepEqual(body, { code: 'EMAIL_CONFIRMATION_REQUIRED', delivery: scenario.delivery })
