...
test('actual submit hook blocks malformed 200 responses and only notifies after validated session changes', async () => {
  ...
  let payload, status = 200, notifications = 0, redirects = 0
  const errors = []
  try {
    Module._load = function(name, ...args) {
    ...
    global.window = { location: { replace: () => { redirects++ } } }
    ...
    status = 409; payload = { code: 'EXISTING_STUDENT_SESSION', next: '/account' }
