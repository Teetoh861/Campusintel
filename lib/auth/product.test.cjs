...
async function invoke(route, providerError, failConfiguration = false) {
  ...
  const mocks = {
    ...
    '@/lib/supabase/auth-gateway': { getAuthGateway: () => ({
      ...
      resend: async () => ({ error: providerError }),
    }) },
  }
  try {
...
test('invalid credentials remain generic; only explicit unconfirmed result enters flow', async () => {
  ...
  assert.deepEqual(await invoke('login', { code: 'email_not_confirmed', message: 'private detail' }), { status: 200, body: { code: EMAIL_CONFIRMATION_REQUIRED, delivery: 'failed' } })
  assert.equal(mapAuthFailure({ message: 'email_not_confirmed' }, 'login').status, 503)
})
test('resend recipient outcomes are identical and provider outages stay safe', async () => {
  const expected = { status: 200, body: { message: AUTH_MESSAGES.email } }
  for (const error of [null, { code: 'user_not_found' }, { code: 'email_exists' }, { code: 'user_already_exists' }, { code: 'over_email_send_rate_limit' }]) {
    assert.deepEqual(await invoke('resend-confirmation', error), expected)
  }
  assert.deepEqual(await invoke('resend-confirmation', { code: 'unexpected_failure', message: 'database error' }), { status: 503, body: { error: AUTH_MESSAGES.unavailable } })
})
