...
async function runScenario({ otpError = null, updateError = null, throws = false, transferFails = false, revokeFails = false } = {}) {
  ...
  const mocks = {
    ...
    '@/lib/supabase/auth-gateway': { getAuthGateway: () => ({ verifyOtp: async params => {
      ...
      calls.push('verify')
      if (otpError) return { data: { session: null, user: null }, error: otpError }
...
test('invalid recovery OTP signals code entry without establishing a session', async () => {
  const { response, body, calls, scopes } = await runScenario({ otpError: { code: 'otp_expired' } })
  assert.equal(response.status, 400)
  assert.equal(body.code, 'RECOVERY_CODE_INVALID')
